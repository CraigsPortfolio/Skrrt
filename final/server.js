...
app.post('/dologin', function(req, res) {
  ...
  db.collection('profiles').findOne({"login.username":uname}, function(err, result) {
    ...
    if(result.login.pword != pword){res.redirect('/main', {logmsg:"Incorrect login details"});res.type('.js');res.send("document.getElementById('id01').style.display='block'");return}
