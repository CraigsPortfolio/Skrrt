...
app.get('/garage', function(req, res) {
  db.collection('profiles').find({"login.username":currentUser}, function(err, result) {
    if (err) throw err;//if there is an error, throw the error
    res.render('pages/garage', {make:result.car[0].make, model:result.car.model, reg:result.car.reg, ftype:result.car.ftype, mpg:result.car.mpg, options:result.car.reg});
  });
});
